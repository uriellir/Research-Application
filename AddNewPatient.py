...
"    }")
        ...
        self.CBoxPatGroup_2.currentTextChanged.connect(self.resName_changed)
    ...
    def resName_changed(self,value):
        ...
        if not(value==''):
            ...
            comoboBox_data = list(dict.fromkeys(patient.get_treshold_Dsection_from_JSON(self.CBoxPatGroup_2.currentText(),'groups')))  # Remove duplicates from list - dictionary cannot have duplicates.
    ...
    def btn_Save_new_patient_clicked(self):
        ...
        if ((self.PatientID.text()=='') or (self.PatName.text()=='') or (self.CBoxPatGroup_2.currentText()=='') or (self.CBoxPatGroup.currentText()=='') or  (self.TxtPath.text()=='')):
            ...
            for condition in ['B','C','D']:
                print("Add -> start")
                pandas_dataFrame = patient.data_section(self.TxtPath.text(),patient.get_treshold_Dsection_from_JSON(self.CBoxPatGroup_2.currentText(),'data_section'), condition) # Needed to add data section from the RESEARCH!!!!!! Calculates the matrix based on the data section.
                print("Add -> after data section")
                correlation_matrix = patient.correlation_matrix(pandas_dataFrame) # Calculates the Correlation Matrix.
                print("Add -> after corelation")
                binary_matrix = patient.binary_matrix(correlation_matrix, patient.get_treshold_Dsection_from_JSON(self.CBoxPatGroup_2.currentText(),'treshold'), condition) # Needed to add treshold from the RESEARCH!!!!! Calculates the Binary matrix based on the treshold.
                print("Add -> after bunary matrix")
            # ------------------------------------#
            em = QtWidgets.QErrorMessage()
